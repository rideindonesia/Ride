...
router.patch("/mitra/:email/status", requireAdmin, async (req, res) => {
  ...
  const [application] = await db.select().from(mitraApplicationsTable)
    .where(eq(mitraApplicationsTable.email, email)).limit(1);
  if (!application) { res.status(404).json({ error: "Lamaran mitra tidak ditemukan" }); return; }

  if (status === "approved") {
    // Email unik di tabel users — pastikan tidak bentrok dengan akun non-mitra
    ...
    if (existingUser && existingUser.role !== "mitra") {
      res.status(409).json({ error: "Email sudah dipakai akun non-mitra, tidak bisa disetujui sebagai mitra" });
      return;
    }

    // Atomik: buat akun login mitra (jika belum ada) + ubah status lamaran bersamaan
    try {
      await db.transaction(async (tx) => {
        ...
        await tx.update(mitraApplicationsTable).set({ status }).where(eq(mitraApplicationsTable.email, email));
      });
    } catch (err) {
      req.log.error({ err, email }, "Gagal membuat akun mitra saat approve");
      res.status(409).json({ error: "Gagal membuat akun mitra — email mungkin sudah dipakai" });
      return;
    }
...
router.patch("/orders/:id/cancel", requireAdmin, async (req, res) => {
  ...
  const [cancelled] = await db.update(ordersTable)
    ...
    .where(and(eq(ordersTable.id, id), inArray(ordersTable.status, ["pending", "accepted"] as [string, ...string[]])))
    .returning({ id: ordersTable.id, penggunaId: ordersTable.penggunaId, mitraId: ordersTable.mitraId, serviceType: ordersTable.serviceType });

  // Notify both parties in real-time so their apps don't stay stuck on an active order.
  if (cancelled) {
    try {
      const payload = { orderId: cancelled.id, canceledBy: "admin", cancelReason };
      io?.to(`order:${cancelled.id}`).emit("order:cancelled", payload);
      io?.to(`user:${cancelled.penggunaId}`).emit("order:cancelled", payload);
      if (cancelled.mitraId) io?.to(`user:${cancelled.mitraId}`).emit("order:cancelled", payload);
      else if (cancelled.serviceType) io?.to(`service:${cancelled.serviceType}`).emit("order:cancelled", payload);
      io?.to("room:admin").emit("admin:order_update", { type: "cancelled", orderId: cancelled.id });
      const targets = [cancelled.penggunaId, ...(cancelled.mitraId ? [cancelled.mitraId] : [])];
      sendPushToUsers(targets, {
        title: "❌ Pesanan Dibatalkan",
        body: cancelReason ? `Alasan: ${cancelReason}` : "Pesanan dibatalkan oleh admin.",
        url: "/",
      }, "pesanan");
    } catch { /* ignore */ }
  }
