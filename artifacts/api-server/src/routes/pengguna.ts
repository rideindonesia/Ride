  ...
  if (voucherCode) {
    ...
    if (voucher) {
      ...
      if (withinUsageLimit && total >= voucher.minOrder) {
        ...
        // Catat pemakaian voucher untuk riwayat pengguna
        try {
        ...
        } catch { /* jangan blok konfirmasi bayar */ }
      }
    }
  }
  ...
  if (cancelled) {
    try {
      ...
      if (cancelled.mitraId) {
        ...
        // Mitra socket bergabung di room `user:<id>` (lihat socket.ts), bukan `mitra:<id>`.
        io?.to(`user:${cancelled.mitraId}`).emit("order:cancelled", { orderId: cancelled.id, canceledBy: "pengguna", cancelReason });
...
// GET /api/pengguna/login-history — riwayat login (perangkat + waktu)
router.get("/login-history", async (req, res) => {
  ...
  const rows = await db.select({
    ...
    .where(and(eq(loginHistoryTable.userId, penggunaId), eq(loginHistoryTable.role, "pengguna")))
    .orderBy(desc(loginHistoryTable.createdAt))
    .limit(20);
  res.json({ history: rows });
});

// GET /api/pengguna/addresses — daftar alamat tersimpan
router.get("/addresses", async (req, res) => {
  const penggunaId = getPenggunaId(req);
  if (!penggunaId) { res.status(401).json({ error: "Belum login" }); return; }
  const rows = await db.select().from(userAddressesTable)
    .where(eq(userAddressesTable.penggunaId, penggunaId))
    .orderBy(desc(userAddressesTable.isDefault), desc(userAddressesTable.createdAt));
  res.json({ addresses: rows });
});

// POST /api/pengguna/addresses — tambah alamat
router.post("/addresses", async (req, res) => {
  const penggunaId = getPenggunaId(req);
  if (!penggunaId) { res.status(401).json({ error: "Belum login" }); return; }
  const { label, address, lat, lng, isDefault } = req.body as {
    label?: string; address?: string; lat?: number; lng?: number; isDefault?: boolean;
  };
  if (!label?.trim() || !address?.trim()) {
    res.status(400).json({ error: "Label dan alamat wajib diisi" }); return;
  }
  if (isDefault) {
    await db.update(userAddressesTable).set({ isDefault: false })
      .where(eq(userAddressesTable.penggunaId, penggunaId));
  }
  const [row] = await db.insert(userAddressesTable).values({
    penggunaId, label: label.trim(), address: address.trim(),
    lat: lat ?? null, lng: lng ?? null, isDefault: !!isDefault,
  }).returning();
  res.status(201).json({ address: row });
});

// PUT /api/pengguna/addresses/:id — ubah alamat
router.put("/addresses/:id", async (req, res) => {
  ...
  if (label != null) set.label = label.trim();
  if (address != null) set.address = address.trim();
  if (lat !== undefined) set.lat = lat;
  if (lng !== undefined) set.lng = lng;
  if (isDefault !== undefined) set.isDefault = !!isDefault;
  const [row] = await db.update(userAddressesTable).set(set)
    .where(and(eq(userAddressesTable.id, id), eq(userAddressesTable.penggunaId, penggunaId)))
    .returning();
  if (!row) { res.status(404).json({ error: "Alamat tidak ditemukan" }); return; }
  res.json({ address: row });
});

// DELETE /api/pengguna/addresses/:id — hapus alamat
router.delete("/addresses/:id", async (req, res) => {
  const penggunaId = getPenggunaId(req);
  if (!penggunaId) { res.status(401).json({ error: "Belum login" }); return; }
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "ID tidak valid" }); return; }
  const [row] = await db.delete(userAddressesTable)
    .where(and(eq(userAddressesTable.id, id), eq(userAddressesTable.penggunaId, penggunaId)))
...
// GET /api/pengguna/voucher-history — riwayat pemakaian voucher
router.get("/voucher-history", async (req, res) => {
  const penggunaId = getPenggunaId(req);
  if (!penggunaId) { res.status(401).json({ error: "Belum login" }); return; }
  const rows = await db.select({
    id: voucherUsageTable.id,
    code: voucherUsageTable.code,
    discount: voucherUsageTable.discount,
    orderId: voucherUsageTable.orderId,
    usedAt: voucherUsageTable.usedAt,
    orderNo: ordersTable.orderNo,
  }).from(voucherUsageTable)
    .leftJoin(ordersTable, eq(voucherUsageTable.orderId, ordersTable.id))
    .where(eq(voucherUsageTable.penggunaId, penggunaId))
    .orderBy(desc(voucherUsageTable.usedAt))
    .limit(30);
  res.json({ history: rows });
});
...
router.post("/chat/:orderId", async (req, res) => {
  ...
  try {
    ...
    if (order.mitraId) {
      sendPushToUsers([order.mitraId], { title: "💬 Pesan dari Konsumen", body: message.trim().slice(0, 80), url: "/" }, "chat");
    }
  } catch {}
