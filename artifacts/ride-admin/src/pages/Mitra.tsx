...
import { Search, Eye, CheckCircle, XCircle, PauseCircle, ChevronLeft, ChevronRight, Star, Trash2, AlertTriangle, Loader2 } from "lucide-react";
...
export default function Mitra() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<MitraItem | null>(null);
  ...
  const deleteMut = useMutation({
    mutationFn: (email: string) => api.del(`/admin/mitra/${encodeURIComponent(email)}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-mitra"] });
        ...
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4" onClick={() => !deleteMut.isPending && setConfirmDelete(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-start gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-gray-900">Hapus Akun Mitra Permanen?</h3>
                <p className="text-sm text-gray-500 mt-0.5">{confirmDelete.name} • {confirmDelete.email}</p>
              </div>
            </div>
            <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700 mb-4">
              Tindakan ini <strong>tidak dapat dibatalkan</strong>. Akun mitra, lokasi, tagihan platform fee, chat, dan dokumen aplikasi akan dihapus permanen. Riwayat order pelanggan tetap tersimpan (tanpa nama mitra). Jika hanya ingin memblokir sementara, gunakan <strong>Suspend</strong>.
            </div>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={() => deleteMut.mutate(confirmDelete.email)} disabled={deleteMut.isPending}
