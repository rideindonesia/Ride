...
export default function DashboardMitra() {
  ...
  const lastLocSentRef = useRef<number>(0); // throttle: timestamp PATCH lokasi terakhir
  ...
  // Login history mitra (dari server)
  const [mLoginHistory, setMLoginHistory] = useState<{ id: number; ipAddress: string | null; userAgent: string | null; createdAt: string }[]>([]);
  // Riwayat chat lengkap (dari server)
  const [mChatConvos, setMChatConvos] = useState<{ orderId: number; orderNo: string | null; serviceType: string; status: string; penggunaName: string | null; lastMessage: string | null; lastAt: string | null }[]>([]);
  const mFmtUADevice = (ua: string | null) => {
    if (!ua) return "Perangkat tidak dikenal";
    const browser = /Edg/.test(ua) ? "Edge" : /Chrome/.test(ua) ? "Chrome" : /Firefox/.test(ua) ? "Firefox" : /Safari/.test(ua) ? "Safari" : /^curl/.test(ua) ? "CLI" : "Browser";
    const os = /Android/.test(ua) ? "Android" : /iPhone|iPad|iOS/.test(ua) ? "iOS" : /Windows/.test(ua) ? "Windows" : /Mac OS|Macintosh/.test(ua) ? "macOS" : /Linux/.test(ua) ? "Linux" : "";
    return os ? `${browser} · ${os}` : browser;
  };
  const mFmtLoginTime = (iso: string) => {
    try { return new Date(iso).toLocaleString("id-ID", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }) + " WIB"; } catch { return iso; }
  };
  const loadMitraHistory = useCallback(() => {
    fetch(`${BASE}/api/mitra/login-history`, { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (Array.isArray(d?.history)) setMLoginHistory(d.history); })
      .catch(() => {});
    fetch(`${BASE}/api/mitra/chat-history`, { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (Array.isArray(d?.conversations)) setMChatConvos(d.conversations); })
      .catch(() => {});
  }, []);
  ...
  // Feed notifikasi tersimpan dari server (persisten lintas refresh)
  const fetchNotifs = useCallback(async () => {
    try {
      const r = await fetch(`${BASE}/api/push/notifications?role=mitra`, { credentials: "include" });
      if (!r.ok) return;
      const d = await r.json();
      const mapCat = (c: string | null): { type: Notif["type"]; icon: string } => {
        if (c === "chat") return { type: "chat", icon: "💬" };
        if (c === "pesanan") return { type: "order", icon: "📦" };
        if (c === "promo") return { type: "system", icon: "🎉" };
        return { type: "system", icon: "🔔" };
      };
      const serverNotifs: Notif[] = (d.notifications ?? []).map((n: any) => {
        const m = mapCat(n.category);
        return { id: `srv-${n.id}`, type: m.type, icon: m.icon, title: n.title, body: n.body, time: new Date(n.createdAt), read: !!n.read };
      });
      setNotifs(prev => {
        const localOnly = prev.filter(x => !String(x.id).startsWith("srv-"));
  ...
  const openNotif = useCallback(async () => {
    ...
    await fetchNotifs();
    try {
      await fetch(`${BASE}/api/push/notifications/read?role=mitra`, {
  ...
  useEffect(() => {
    ...
    const onOrderCancelled = (data: { orderId: number; canceledBy?: string; cancelReason?: string }) => {
      ...
      // Refetch feed jika konsumen yang batalkan (bukan mitra sendiri)
  ...
  }, [fetchDashboard, fetchIncoming, fetchActiveOrder, fetchNotifs]);
  ...
  // Muat preferensi notifikasi mitra dari server (sumber kebenaran)
  const mNotifLoadedRef = useRef(false);
  useEffect(() => {
    fetch("/api/push/prefs?role=mitra", { credentials: "include" })
      .then(r => (r.ok ? r.json() : null))
      .then(d => { if (d?.prefs && Object.keys(d.prefs).length) setMNotifSettings((s: any) => ({ ...s, ...d.prefs })); })
      .catch(() => {})
      .finally(() => { mNotifLoadedRef.current = true; });
  }, []);

  // Sync mitra notif settings ke localStorage + server
  useEffect(() => {
    localStorage.setItem("ride-notif-m", JSON.stringify(mNotifSettings));
    if (mNotifLoadedRef.current) {
      fetch("/api/push/prefs?role=mitra", {
        method: "PUT", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prefs: mNotifSettings }),
      }).catch(() => {});
    }
  }, [mNotifSettings]);
  ...
  // GPS WAJIB untuk mitra — minta posisi, reject jika ditolak/gagal/tidak tersedia
  const requestGps = (): Promise<GeolocationPosition> => new Promise((resolve, reject) => {
    if (!navigator.geolocation) { reject(new Error("no-geolocation")); return; }
    let settled = false;
    // Hard timeout: jaminan reject jika browser tidak memanggil success/error (mis. izin pending)
    const timer = setTimeout(() => { if (!settled) { settled = true; reject(new Error("gps-timeout")); } }, 9000);
    navigator.geolocation.getCurrentPosition(
  ...
  const sendOnlineState = async (online: boolean, lat?: number, lng?: number) => {
    const res = await fetch(`${BASE}/api/mitra/toggle-online`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ isOnline: online, lat, lng }),
    });
  ...
  const toggleOnline = async () => {
    ...
    const next = !isOnline;
    if (next) {
      // Tidak boleh online tanpa GPS aktif
      setTogglingOnline(true);
      let pos: GeolocationPosition;
      try { pos = await requestGps(); }
      catch { setTogglingOnline(false); setGpsBlocked(true); return; }
      try {
        await sendOnlineState(true, pos.coords.latitude, pos.coords.longitude);
        setIsOnline(true);
      } catch { /* tetap offline */ }
      finally { setTogglingOnline(false); }
    } else {
      setTogglingOnline(true);
      try { await sendOnlineState(false); setIsOnline(false); }
      catch { /* abaikan */ }
      finally { setTogglingOnline(false); }
    }
  };
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      if (r.status === 401) {
        pushNotif({ type: "system", icon: "🔒", title: "Sesi Berakhir", body: "Sesi Anda telah habis. Silakan login ulang." });
        navigate("/");
        return;
      }
  ...
  // Hentikan live tracking lokasi mitra (stabil — hanya menyentuh ref)
  ...
  const beginTracking = useCallback((pLat: number | null, pLng: number | null) => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => { startCountdown(600); setGpsBlocked(true); }, // GPS ditolak saat menuju — paksa aktifkan lagi
      ...
      locationWatchRef.current = navigator.geolocation.watchPosition(
        pos => {
          ...
          startCountdown(calcEtaSecsLive(km, speedKmh));
          // Throttle PATCH lokasi ke server: maksimal 1× / 8 detik agar tidak membanjiri backend
          const now = Date.now();
          if (now - lastLocSentRef.current >= 8000) {
  ...
  // Coba aktifkan GPS lagi dari modal — jika berhasil, lanjutkan online / tracking
  const retryEnableGps = async () => {
    setRetryingGps(true);
    try {
      const pos = await requestGps();
      setGpsBlocked(false);
      if (mitraPhase === "menuju" && activeOrder?.pickupLat && activeOrder?.pickupLng) {
        beginTracking(activeOrder.pickupLat, activeOrder.pickupLng);
      } else if (!isBusyWithOrder && !isOnline) {
        try { await sendOnlineState(true, pos.coords.latitude, pos.coords.longitude); setIsOnline(true); } catch { /* abaikan */ }
      }
    } catch {
      showToast({ icon: "📍", title: "GPS masih nonaktif", body: "Aktifkan izin lokasi di perangkat Anda, lalu coba lagi.", color: "red" });
    } finally { setRetryingGps(false); }
  };

  // GPS WAJIB: pantau izin lokasi — evaluasi state awal + pencabutan izin → paksa offline + modal
  useEffect(() => {
    ...
    const enforce = () => {
      if (st && st.state === "denied") {
        ...
        setIsOnline(false);
        stopLocationWatch();
        fetch(`${BASE}/api/mitra/toggle-online`, {
          method: "PATCH", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ isOnline: false }),
        }).catch(() => {});
      }
    };
    anyNav.permissions.query({ name: "geolocation" }).then((s) => {
      if (cancelled) return; // effect sudah di-cleanup sebelum promise selesai
      st = s;
      enforce(); // tegakkan jika sudah denied sejak awal (mis. sesi online dipulihkan dari server)
      s.addEventListener("change", enforce);
    }).catch(() => {});
    return () => { cancelled = true; if (st) st.removeEventListener("change", enforce); };
  }, [stopLocationWatch]);
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", background: "#f0f4f8", overflow: "hidden", position: "relative" }}>
      ...
      {/* ── GPS Wajib Modal (Mitra) ── */}
      {gpsBlocked && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 6000, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 20px" }}>
          <div style={{ background: "#fff", borderRadius: 22, padding: "26px 22px 22px", width: "100%", maxWidth: 380, boxShadow: "0 8px 32px rgba(0,0,0,0.18)", textAlign: "center" }}>
            <div style={{ width: 64, height: 64, borderRadius: 32, background: "#fef2f2", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", fontSize: 30 }}>📍</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#1a2a3a", marginBottom: 8 }}>Aktifkan GPS Anda</div>
            <div style={{ fontSize: 13.5, color: "#5a6a7a", lineHeight: 1.5, marginBottom: 20 }}>
              GPS <b>wajib aktif</b> agar Anda bisa online dan menerima pesanan. Lokasi Anda ditampilkan ke pengguna selama mengantar layanan. Mohon izinkan akses lokasi di perangkat Anda.
            </div>
            <button onClick={retryEnableGps} disabled={retryingGps}
              style={{ width: "100%", padding: "13px", borderRadius: 14, border: "none", background: retryingGps ? "#9ca3af" : "#1a7a6a", color: "#fff", fontSize: 15, fontWeight: 700, cursor: retryingGps ? "default" : "pointer", marginBottom: 10 }}>
              {retryingGps ? "Memeriksa..." : "Aktifkan GPS Sekarang"}
            </button>
            {!isBusyWithOrder && mitraPhase !== "menuju" ? (
              <button onClick={() => setGpsBlocked(false)} disabled={retryingGps}
                style={{ width: "100%", padding: "10px", borderRadius: 12, border: "none", background: "transparent", color: "#94a3b8", fontSize: 13, fontWeight: 600, cursor: retryingGps ? "default" : "pointer" }}>
                Nanti saja
              </button>
            ) : (
              <div style={{ fontSize: 12, color: "#ef4444", fontWeight: 600, marginTop: 2 }}>
                GPS wajib aktif selama Anda mengantar pesanan
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 90px" }}>
        ...
        {activeTab === "chat" && <>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
            {([
              ...
              { id: "riwayat" as const, label: "Riwayat Chat", count: mChatConvos.length },
            ]).map(tab => (
            ...
            {mChatConvos.length === 0 ? (
              ...
              mChatConvos.map((o, i) => (
                <div key={o.orderId}>
                  ...
                  <div>
                    <button onClick={() => fetchChatHistory(o.orderId)} style={{ width: "100%", padding: "14px 16px", border: "none", background: "transparent", cursor: "pointer", display: "flex", gap: 12, alignItems: "center", textAlign: "left" as const }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: "#1a2a3a" }}>{o.penggunaName ?? "Konsumen"}</div>
                        <div style={{ fontSize: 12, color: "#7a8a9a" }}>{o.orderNo ?? `#${o.orderId}`}{o.lastAt ? ` · ${fmtDate(o.lastAt)}` : ""}</div>
                        {o.lastMessage && <div style={{ fontSize: 11, color: "#9aa5b4", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{o.lastMessage}</div>}
                      </div>
                      <span style={{ fontSize: 14, color: "#1a7a6a", fontWeight: 700 }}>{chatHistoryOrderId === o.orderId ? "▲" : "▼"}</span>
                    </button>
                    {chatHistoryOrderId === o.orderId && (
        ...
        {activeTab === "akun" && (() => {
          ...
          <div style={{ padding: "0 0 12px" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 18, boxShadow: "0 2px 8px rgba(0,0,0,0.06)", marginBottom: 10, overflow: "hidden" }}>
              ...
              <div>
                ...
                {mKeamananSub === "riwayat-login" && (
                  <div style={{ padding:"0 14px 14px", borderTop:"1px solid #f0f4f8" }}>
                    {mLoginHistory.length === 0 && <div style={{ fontSize:12, color:"#9aa5b4", padding:"8px 0" }}>Belum ada riwayat login.</div>}
                    {mLoginHistory.map((item, i) => {
                      const device = mFmtUADevice(item.userAgent);
                      return (
                      <div key={item.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderTop: i>0?"1px solid #f0f4f8":"none" }}>
                        <div style={{ width:36, height:36, borderRadius:10, background: i===0?"#e8f5f2":"#f0f4f8", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>
                          {/Android|iOS/.test(device)?"📱":"💻"}
                        </div>
                        <div style={{ flex:1 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                            <div style={{ fontSize:12, fontWeight:700, color:"#1a2a3a" }}>{device}</div>
                            {i===0 && <span style={{ background:"#1a7a6a", color:"#fff", fontSize:9, fontWeight:800, borderRadius:5, padding:"1px 6px" }}>SAAT INI</span>}
                          </div>
                          <div style={{ fontSize:11, color:"#9aa5b4", marginTop:2 }}>{mFmtLoginTime(item.createdAt)}{item.ipAddress ? ` · ${item.ipAddress}` : ""}</div>
                        </div>
                      </div>
                      );
                    })}
            ...
            {/* Tombol Keluar */}
            <button onClick={async () => { try { localStorage.removeItem("ride-last-role"); } catch { /* ignore */ } await fetch(`${BASE}/api/auth/logout?role=mitra`, { method:"POST", credentials:"include" }); navigate("/"); }}
