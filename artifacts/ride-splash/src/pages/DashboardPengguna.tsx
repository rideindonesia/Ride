...
export default function DashboardPengguna() {
  ...
  const [notifList, setNotifList] = useState<{ id: number; title: string; body: string; url: string | null; category: string | null; read: boolean; createdAt: string }[]>([]);
  const [notifUnread, setNotifUnread] = useState(0);
  const fetchNotifs = async () => {
    try {
      const r = await fetch("/api/push/notifications?role=pengguna", { credentials: "include" });
      if (!r.ok) return;
  ...
  const openNotif = async () => {
    ...
    await fetchNotifs();
    try {
      await fetch("/api/push/notifications/read?role=pengguna", {
  ...
  const [alamatList, setAlamatList] = useState<{ id: number; label: string; address: string; isDefault: boolean }[]>([]);
  const loadAddresses = useCallback(() => {
    fetch("/api/pengguna/addresses", { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (Array.isArray(d?.addresses)) setAlamatList(d.addresses); })
      .catch(() => {});
  }, []);
  ...
  // Default address — turunan dari data server (kolom isDefault)
  const defaultAlamatId = alamatList.find(a => a.isDefault)?.id ?? null;

  // Format perangkat dari user-agent & waktu login
  const fmtUADevice = (ua: string | null) => {
  ...
  // Login history (dari server)
  const [loginHistory, setLoginHistory] = useState<{ id: number; ipAddress: string | null; userAgent: string | null; createdAt: string }[]>([]);

  // Voucher usage history (dari server)
  const [voucherHistory, setVoucherHistory] = useState<{ id: number; code: string; discount: number; orderNo: string | null; usedAt: string }[]>([]);
  ...
  // Muat preferensi notifikasi dari server (sumber kebenaran), merge ke default
  const notifLoadedRef = useRef(false);
  useEffect(() => {
    fetch("/api/push/prefs?role=pengguna", { credentials: "include" })
      .then(r => (r.ok ? r.json() : null))
      .then(d => { if (d?.prefs && Object.keys(d.prefs).length) setNotifSettings((s: any) => ({ ...s, ...d.prefs })); })
      .catch(() => {})
      .finally(() => { notifLoadedRef.current = true; });
  }, []);

  // Sync notif settings ke localStorage + server
  useEffect(() => {
    localStorage.setItem("ride-notif-p", JSON.stringify(notifSettings));
    if (notifLoadedRef.current) {
      fetch("/api/push/prefs?role=pengguna", {
        method: "PUT", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prefs: notifSettings }),
      }).catch(() => {});
    }
  }, [notifSettings]);

  // Muat alamat, riwayat login, & riwayat voucher dari server saat mount
  useEffect(() => {
    loadAddresses();
    fetch("/api/pengguna/login-history", { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (Array.isArray(d?.history)) setLoginHistory(d.history); })
      .catch(() => {});
    fetch("/api/pengguna/voucher-history", { credentials: "include" })
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (Array.isArray(d?.history)) setVoucherHistory(d.history); })
      .catch(() => {});
  }, [loadAddresses]);
    ...
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", background: "#f0f4f8", fontFamily: "'Inter', sans-serif", overflow: "hidden" }}>
      ...
      {activeTab === "beranda" && <div style={{ background: "linear-gradient(160deg, #0d2137 0%, #1a3a5c 60%, #1a7a6a 100%)", padding: "48px 14px 16px", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 14, background: "rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
              {profile?.profilePhotoPath
                ? <img src={profile.profilePhotoPath} alt="foto" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          ...
          <button onClick={openNotif} aria-label="Notifikasi" style={{ position: "relative", width: 44, height: 44, borderRadius: 14, background: "rgba(255,255,255,0.15)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 22 }}>🔔</span>
            {notifUnread > 0 && (
              <span style={{ position: "absolute", top: -4, right: -4, background: "#e74c3c", color: "#fff", fontSize: 10, fontWeight: 700, borderRadius: 10, padding: "1px 5px", minWidth: 18, textAlign: "center" }}>{notifUnread}</span>
            )}
          </button>
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 80 }}>
        ...
        {activeTab === "akun" && <div style={{ padding: "16px 10px 12px" }}>
              ...
              {openAkunSection === "voucher" && (
                <div style={{ padding: "0 14px 14px", borderTop: "1px solid #f0f4f8" }}>
                  ...
                  {voucherHistory.map((v) => (
                    <div key={v.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderTop: "1px solid #f0f4f8" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 10, color: "#5a7a6a" }}>Hemat Rp {v.discount.toLocaleString("id-ID")}{v.orderNo ? ` · ${v.orderNo}` : ""}</div>
                      </div>
                      <div style={{ fontSize: 10, color: "#9aa5b4" }}>{fmtLoginTime(v.usedAt)}</div>
                    </div>
                  ))}
                </div>
              )}
                ...
                {openAkunSection === "alamat" && item.id === "alamat" && (
                  <div style={{ padding: "0 14px 14px", borderTop: "1px solid #f0f4f8" }}>
                          ...
                          <div>
                            ...
                            <div style={{ display: "flex", gap: 6 }}>
                              <button onClick={async () => {
                                ...
                                await fetch(`/api/pengguna/addresses/${a.id}`, { method: "PUT", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ label: editAlamatLabel.trim(), address: editAlamatAddr.trim() }) }).catch(() => {});
                          ...
                          <>
                            <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                              ...
                              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                                ...
                                <button onClick={async () => { await fetch(`/api/pengguna/addresses/${a.id}`, { method: "DELETE", credentials: "include" }).catch(() => {}); loadAddresses(); }}
                                  style={{ background: "none", border: "none", color: "#e74c3c", fontSize: 16, cursor: "pointer", padding: 0 }}>×</button>
                              </div>
                            </div>
                            {a.id !== defaultAlamatId && (
                              <button onClick={async () => { await fetch(`/api/pengguna/addresses/${a.id}`, { method: "PUT", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isDefault: true }) }).catch(() => {}); loadAddresses(); }}
                    ...
                    <div style={{ marginTop: 10 }}>
                      ...
                      <button onClick={async () => { if (!newAlamatLabel.trim() || !newAlamatAddr.trim()) return; await fetch("/api/pengguna/addresses", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ label: newAlamatLabel.trim(), address: newAlamatAddr.trim(), isDefault: alamatList.length === 0 }) }).catch(() => {}); setNewAlamatLabel(""); setNewAlamatAddr(""); loadAddresses(); }}
          ...
          <div style={{ background: "#fff", borderRadius: 18, boxShadow: "0 2px 8px rgba(0,0,0,0.06)", marginBottom: 10, overflow: "hidden" }}>
            ...
            <div>
              ...
              {keamananSubSection === "riwayat-login" && (
                <div style={{ padding: "0 14px 14px", borderTop: "1px solid #f0f4f8" }}>
                  {loginHistory.length === 0 && <div style={{ fontSize: 12, color: "#9aa5b4", padding: "8px 0" }}>Belum ada riwayat login.</div>}
                  {loginHistory.map((item, i) => {
                    const device = fmtUADevice(item.userAgent);
                    return (
                    <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderTop: i > 0 ? "1px solid #f0f4f8" : "none" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: "#9aa5b4", marginTop: 2 }}>{fmtLoginTime(item.createdAt)}{item.ipAddress ? ` · ${item.ipAddress}` : ""}</div>
                      </div>
                    </div>
                    );
                  })}
          ...
          {/* Tombol Keluar */}
          <button onClick={() => { try { localStorage.removeItem("ride-last-role"); } catch { /* ignore */ } fetch("/api/auth/logout?role=pengguna", { method: "POST", credentials: "include" }).then(() => navigate("/login")); }}
      ...
      {/* Panel Notifikasi */}
      {notifOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 6000, display: "flex", flexDirection: "column" }}>
          <div onClick={() => setNotifOpen(false)} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.45)" }} />
          <div style={{ position: "relative", marginTop: "auto", background: "#f0f4f8", borderTopLeftRadius: 20, borderTopRightRadius: 20, maxHeight: "80vh", display: "flex", flexDirection: "column", boxShadow: "0 -4px 24px rgba(0,0,0,0.2)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 18px 12px", borderBottom: "1px solid #e2e8f0" }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: "#0d2137" }}>Notifikasi</div>
              <button onClick={() => setNotifOpen(false)} style={{ width: 32, height: 32, borderRadius: 10, background: "#e2e8f0", border: "none", cursor: "pointer", fontSize: 16, fontWeight: 700, color: "#475569" }}>✕</button>
            </div>
            <div style={{ overflowY: "auto", padding: "10px 14px 24px" }}>
              {notifList.length === 0 ? (
                ...
                notifList.map(n => (
                  <div key={n.id}
                    onClick={() => { if (n.url) { setNotifOpen(false); navigate(n.url); } }}
                    style={{ background: "#fff", borderRadius: 14, padding: "12px 14px", marginBottom: 8, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", borderLeft: n.read ? "3px solid transparent" : "3px solid #1a7a6a", cursor: n.url ? "pointer" : "default" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 8, marginBottom: 3 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#0d2137", lineHeight: 1.3 }}>{n.title}</div>
                      {!n.read && <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#e74c3c", flexShrink: 0, marginTop: 5 }} />}
                    </div>
                    <div style={{ fontSize: 13, color: "#475569", lineHeight: 1.4 }}>{n.body}</div>
                    <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 5 }}>{relativeTime(n.createdAt)}</div>
                  </div>
                ))
              )}
            </div>
