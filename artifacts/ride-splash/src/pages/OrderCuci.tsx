...
const PAKET_CUCI: Record<"motor" | "mobil" | "pickup", string[]> = {
  motor:  ["Cuci Biasa", "Cuci + Semir Ban", "Cuci + Wax Kilap"],
  mobil:  ["Exterior Biasa", "Exterior Premium", "Interior + Exterior", "Full Detailing"],
  pickup: ["Exterior Biasa", "Exterior Premium", "Interior + Exterior"],
};
...
const ACCENT = "#1a5c7c";
const ACCENT2 = "#0e3a5c";

export default function OrderCuci() {
  ...
  useEffect(() => {
    ...
    if (!trackLeafletRef.current) {
      ...
      const uIcon = L.divIcon({ html: '<div style="width:28px;height:28px;background:#e53e3e;border:3px solid #fff;border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;font-size:14px;">📍</div>', iconSize: [28,28], iconAnchor: [14,28], className: "" });
      trackUserMarkerRef.current = L.marker([pinLat, pinLng], { icon: uIcon }).addTo(map).bindPopup("Lokasi Anda");
      const mIcon = L.divIcon({ html: '<div style="width:34px;height:34px;background:#1a5c7c;border:3px solid #fff;border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center;font-size:16px;">🧹</div>', iconSize: [34,34], iconAnchor: [17,17], className: "" });
      if (mitraTrackLat && mitraTrackLng) { trackMitraMarkerRef.current = L.marker([mitraTrackLat, mitraTrackLng], { icon: mIcon }).addTo(map).bindPopup("Petugas"); map.fitBounds(L.latLngBounds([[pinLat,pinLng],[mitraTrackLat,mitraTrackLng]]), { padding: [40,40] }); }
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", background: "#f0f4f8", overflow: "hidden" }}>
      <div style={{ background: `linear-gradient(160deg, #0d2137 0%, #1a3a5c 60%, ${ACCENT} 100%)`, padding: "52px 14px 0", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <div style={{ color: "#fff", fontSize: 17, fontWeight: 700 }}>🚿 Cuci Kendaraan</div>
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 100px" }}>
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "22px 16px" }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#1a2a3a", marginBottom: 20 }}>🚿 Pilih Paket</div>
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#4a5568", display: "block", marginBottom: 10 }}>Jenis Kendaraan</label>
                <div style={{ display: "flex", gap: 10 }}>
                  {(["motor","mobil","pickup"] as const).map(j => (
                    <button key={j} onClick={() => handleJenisChange(j)}
                      style={{ flex: 1, padding: "13px 8px", borderRadius: 14, border: jenisKendaraan === j ? `2px solid ${ACCENT}` : "2px solid #e0e8f0", background: jenisKendaraan === j ? `rgba(26,92,124,0.08)` : "#f8fafc", color: jenisKendaraan === j ? ACCENT : "#7a8a9a", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                      {j === "motor" ? "Motor" : j === "mobil" ? "Mobil" : "Pick Up"}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {PAKET_CUCI[jenisKendaraan].map(p => (
                    <button key={p} onClick={() => setPaketCuci(p)}
                      style={{ width: "100%", padding: "16px 18px", borderRadius: 14, border: paketCuci === p ? `2px solid ${ACCENT}` : "1.5px solid #e0e8f0", background: paketCuci === p ? `rgba(26,92,124,0.07)` : "#fff", color: paketCuci === p ? ACCENT : "#1a2a3a", fontWeight: paketCuci === p ? 700 : 500, fontSize: 15, cursor: "pointer", textAlign: "left" as const, transition: "all 0.15s" }}>
                      {p}
                    </button>
                  ))}
                </div>
                <div style={{ fontSize: 12, color: "#9aa5b4", marginTop: 10, fontStyle: "italic" }}>*Biaya jasa akan didiskusikan via chat dengan petugas cuci</div>
              </div>
            </div>
          </div>
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 100px" }}>
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "20px 14px" }}>
              ...
              {autoAddress && <div style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "10px 14px", background: "rgba(26,122,106,0.07)", borderRadius: 12, marginBottom: 16, border: "1px solid rgba(26,122,106,0.2)" }}><span style={{ fontSize: 14, flexShrink: 0, marginTop: 1 }}>📍</span><span style={{ fontSize: 13, color: "#1a3a5c", lineHeight: 1.4 }}>{autoAddress}</span></div>}
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#4a5568", display: "block", marginBottom: 8 }}>Detail Alamat</label>
                <textarea value={detailAlamat} onChange={e => setDetailAlamat(e.target.value)} placeholder="Depan rumah / area parkir / dll..." rows={3} style={{ width: "100%", padding: "14px 16px", borderRadius: 14, border: "1.5px solid #e0e8f0", fontSize: 15, color: "#1a2a3a", background: "#f8fafc", outline: "none", resize: "none", lineHeight: 1.5 }} />
              </div>
            </div>
          </div>
      ...
      {step === 3 && (
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 100px" }}>
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "22px 16px" }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#1a2a3a", marginBottom: 24 }}>🧹 Cari Petugas</div>
              ...
              {orderStatus === "accepted" && acceptedMitra && (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 18px", background: "#e8f8f2", borderRadius: 14, border: "1.5px solid #b2e8d4" }}><span style={{ fontSize: 22 }}>✅</span><div style={{ fontSize: 15, fontWeight: 700, color: "#1a7a6a" }}>Petugas Cuci Ditemukan!</div></div>
                  ...
                  {chatOpen && (
                    <div style={{ border: "1.5px solid #e0e8f0", borderRadius: 16, overflow: "hidden" }}>
                      ...
                      <div style={{ minHeight: 160, maxHeight: 220, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: 8, background: "#fff" }}>
                        {chatMessages.length === 0 ? (<div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "20px 0" }}><span style={{ fontSize: 32, opacity: 0.3 }}>💬</span><div style={{ fontSize: 12, color: "#b0bec5" }}>Mulai diskusi dengan petugas</div></div>) : chatMessages.map(m => { const isMine = m.senderRole === "pengguna"; return (<div key={m.id} style={{ display: "flex", flexDirection: "column", alignItems: isMine ? "flex-end" : "flex-start" }}><div style={{ maxWidth: "78%", padding: "8px 12px", borderRadius: isMine ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: isMine ? `linear-gradient(135deg,${ACCENT2},${ACCENT})` : "#f0f4f8", color: isMine ? "#fff" : "#1a2a3a", fontSize: 13 }}>{m.message}</div></div>); })}
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, padding: "12px 14px 20px", background: "linear-gradient(to top, #f0f4f8 90%, transparent)", zIndex: 100 }}>
            {orderStatus === "accepted" ? (<div style={{ display: "flex", gap: 12 }}><button onClick={async () => { if (orderId) await fetch(`/api/pengguna/orders/${orderId}`, { method: "DELETE", credentials: "include" }).catch(() => {}); if (orderPollRef.current) clearInterval(orderPollRef.current); if (chatPollRef.current) clearInterval(chatPollRef.current); navigate("/dashboard/pengguna"); }} style={{ flex: 1, padding: "15px", borderRadius: 16, border: "1.5px solid #e8a0a0", background: "#fff5f5", color: "#c0392b", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>✕ Batalkan</button><button onClick={() => { if (mitraConfirmed) { setChatOpen(false); setStep(4); } }} disabled={!mitraConfirmed} style={{ flex: 2, padding: "15px", borderRadius: 16, border: "none", background: mitraConfirmed ? `linear-gradient(135deg, ${ACCENT2} 0%, ${ACCENT} 100%)` : "#d0d8e0", color: mitraConfirmed ? "#fff" : "#a0aab4", fontWeight: 700, fontSize: 15, cursor: mitraConfirmed ? "pointer" : "not-allowed" }}>Lanjut →</button></div>)
              : (<button disabled style={{ width: "100%", padding: "17px", borderRadius: 16, border: "none", background: "#c0d0dc", color: "#fff", fontWeight: 700, fontSize: 16, cursor: "not-allowed" }}>{orderStatus === "creating" ? "Membuat pesanan..." : "Menunggu Petugas Menerima..."}</button>)}
          </div>
        </>
      )}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "0 0 100px" }}>
            <div style={{ position: "relative", width: "100%", height: 220 }}>
              ...
              <div style={{ position: "absolute", top: 12, right: 12, zIndex: 500, display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <div style={{ background: `rgba(26,92,124,0.9)`, backdropFilter: "blur(6px)", borderRadius: 12, padding: "8px 14px", color: "#fff", fontSize: 12, fontWeight: 600 }}>📏 {trackDist != null ? (trackDist < 1 ? `${Math.round(trackDist*1000)} m` : `${trackDist.toFixed(1)} km`) : `${acceptedMitra.dist < 1 ? `${Math.round(acceptedMitra.dist*1000)} m` : `${acceptedMitra.dist.toFixed(1)} km`}`}</div>
              </div>
              <div style={{ position: "absolute", top: 12, left: 12, zIndex: 500, background: ACCENT, borderRadius: 12, padding: "8px 14px", color: "#fff", fontSize: 12, fontWeight: 700 }}>🧹 Petugas dalam perjalanan...</div>
            </div>
            <div style={{ padding: "16px 14px 0" }}>
              <div style={{ border: "1.5px solid #e0e8f0", borderRadius: 16, padding: "14px 16px", display: "flex", gap: 12, alignItems: "center", marginBottom: 16, background: "#fff" }}>
                ...
                <div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 700, color: "#1a2a3a" }}>{acceptedMitra.name}</div><div style={{ fontSize: 12, color: "#7a8a9a", marginTop: 2 }}>✅ Petugas Terverifikasi RIDE</div><div style={{ fontSize: 12, color: "#9aa5b4", marginTop: 1 }}>{acceptedMitra.rating != null ? `⭐ ${acceptedMitra.rating}` : "⭐ Baru"} · {acceptedMitra.totalOrders} order</div></div>
              ...
              <div style={{ background: "#fff", border: "1.5px solid #e0e8f0", borderRadius: 16, padding: "16px", marginBottom: 16 }}>
                ...
                {(["menuju","tiba","pengerjaan","selesai"]).map((key, i) => {
                  const labels = ["Petugas menuju lokasi Anda","Petugas sudah tiba","Sedang proses cuci","Proses cuci selesai ✅"];
                  ...
                  return (<div key={key} style={{ display: "flex", gap: 14 }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                      <div style={{ width: 24, height: 24, borderRadius: 12, flexShrink: 0, background: done ? "#1a7a6a" : active ? "#1a3a5c" : "#e0e8f0", display: "flex", alignItems: "center", justifyContent: "center", border: active ? "2px solid #1a7a6a" : "none" }}>
                        {done ? <span style={{ color: "#fff", fontSize: 11 }}>✓</span> : active ? <div style={{ width: 7, height: 7, borderRadius: 4, background: "#1a7a6a" }} /> : <div style={{ width: 6, height: 6, borderRadius: 3, background: "#c0d0dc" }} />}
                      </div>
                      {i < 3 && <div style={{ width: 2, height: 28, background: "#e0e8f0", margin: "3px 0" }} />}
                    </div>
                    <div style={{ paddingBottom: 14 }}><div style={{ fontSize: 13, fontWeight: active ? 700 : 500, color: active ? "#1a2a3a" : "#9aa5b4" }}>{labels[i]}</div>{active && <div style={{ fontSize: 11, color: "#1a7a6a", fontWeight: 600, marginTop: 1 }}>• Sekarang</div>}</div>
                  </div>);
                })}
              </div>
            </div>
      ...
      {step === 5 && (() => {
        ...
        const pmLabel: Record<string,string> = { cash: "Bayar Tunai ke Petugas", transfer: "Transfer Bank", qris: "Bayar via QRIS" };
        const pmDesc: Record<string,string> = { cash: `Siapkan uang tunai sebesar ${fmtIdr(finalTotal)}.`, transfer: `Transfer ke rekening petugas sebesar ${fmtIdr(finalTotal)}.`, qris: `Scan QRIS petugas dan bayar sebesar ${fmtIdr(finalTotal)}.` };
          ...
          <>
            <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 120px" }}>
              <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "20px 16px" }}>
                ...
                <div style={{ background: "#f0faf7", border: "1.5px solid #b6e6d7", borderRadius: 14, padding: "12px 16px", marginBottom: 16, textAlign: "center" as const }}><div style={{ fontSize: 14, fontWeight: 800, color: "#1a7a6a" }}>✅ Kendaraan Sudah Dicuci!</div><div style={{ fontSize: 12, color: "#4a9a7a", marginTop: 3 }}>Silakan selesaikan pembayaran</div></div>
                {!paymentData && <div style={{ background: "#f8fafc", borderRadius: 16, border: "1.5px solid #e0e8f0", padding: "24px 16px", textAlign: "center" as const, marginBottom: 16 }}><div style={{ fontSize: 28, marginBottom: 10, animation: "spin 1s linear infinite", display: "inline-block" }}>⏳</div><div style={{ fontSize: 14, fontWeight: 700, color: "#1a2a3a", marginBottom: 6 }}>Menunggu petugas mengisi rincian biaya...</div></div>}
                {paymentData && !paymentConfirmed && (
                  <>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>{(["cash","transfer","qris"] as const).map(m => (<button key={m} onClick={() => setPaymentMethodUser(m)} style={{ flex: 1, padding: "10px 0", borderRadius: 10, border: selectedMethod === m ? `2px solid ${ACCENT}` : "1.5px solid #e0e8f0", background: selectedMethod === m ? `rgba(26,92,124,0.07)` : "#fff", color: selectedMethod === m ? ACCENT : "#7a8a9a", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>{m === "cash" ? "Cash" : m === "transfer" ? "Transfer" : "QRIS"}</button>))}</div>
