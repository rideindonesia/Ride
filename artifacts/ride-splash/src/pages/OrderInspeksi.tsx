...
const STEPS = [
  { label: "Tujuan", emoji: "🔍" },
  { label: "Lokasi", emoji: "📍" },
  { label: "Inspektor", emoji: "🔬" },
  { label: "Tracking", emoji: "📡" },
  { label: "Laporan", emoji: "📋" },
];

const TUJUAN_INSPEKSI = [
  "Beli Kendaraan Bekas",
  "Cek Kondisi Berkala",
  "Sebelum Jual Kendaraan",
  "Setelah Kecelakaan",
];
...
const ACCENT = "#2a5c2a";
const ACCENT2 = "#1a3c1a";

export default function OrderInspeksi() {
  ...
  const canNext1 = tujuan !== "" && merekModel.trim() !== "";
  ...
  useEffect(() => {
    ...
    const doPoll = async () => {
      try {
        const res = await fetch(`/api/pengguna/orders/${orderId}`, { credentials: "include" }); if (!res.ok) return;
        ...
        if (od.status === "accepted" && od.mitra) {
          ...
          setAcceptedMitra({ id: od.mitra.id, name: od.mitra.name, lat: od.mitra.lat ?? 0, lng: od.mitra.lng ?? 0, serviceType: od.mitra.serviceType, rating: od.mitra.rating ?? null, totalOrders: od.mitra.totalOrders ?? 0, dist, callFee: Math.round((dist*2000+10000)/500)*500, etaMin: Math.max(5, Math.round(dist*2+5)) });
          setOrderStatus("accepted");
        } else if (od.status === "cancelled") { if (orderPollRef.current) clearInterval(orderPollRef.current); setOrderStatus("cancelled"); }
      } catch { }
    };
    doPoll(); orderPollRef.current = setInterval(doPoll, 3000);
  ...
  useEffect(() => {
    ...
    const fetchMsgs = async () => { try { const res = await fetch(`/api/chat/${orderId}`, { credentials: "include" }); if (!res.ok) return; const d = await res.json(); setChatMessages(d.messages ?? []); setTimeout(() => { const el = chatBottomRef.current?.parentElement; if (el) el.scrollTop = el.scrollHeight; }, 50); } catch { } };
    fetchMsgs(); chatPollRef.current = setInterval(fetchMsgs, 3000);
  ...
  useEffect(() => {
    ...
    poll(); step5PollRef.current = setInterval(poll, 3000);
  ...
  useEffect(() => {
    ...
    if (!trackLeafletRef.current) {
      ...
      trackUserMarkerRef.current = L.marker([pinLat, pinLng], { icon: uIcon }).addTo(map).bindPopup("Lokasi Kendaraan");
      const mIcon = L.divIcon({ html: '<div style="width:34px;height:34px;background:#2a5c2a;border:3px solid #fff;border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,0.4);font-size:16px;display:flex;align-items:center;justify-content:center;">🔍</div>', iconSize: [34,34], iconAnchor: [17,17], className: "" });
      if (mitraTrackLat && mitraTrackLng) { trackMitraMarkerRef.current = L.marker([mitraTrackLat, mitraTrackLng], { icon: mIcon }).addTo(map).bindPopup("Inspektor"); map.fitBounds(L.latLngBounds([[pinLat,pinLng],[mitraTrackLat,mitraTrackLng]]), { padding: [40,40] }); }
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", background: "#f0f4f8", overflow: "hidden" }}>
      <div style={{ background: `linear-gradient(160deg, #0d1f0d 0%, #1a3c1a 60%, ${ACCENT} 100%)`, padding: "52px 14px 0", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <div style={{ color: "#fff", fontSize: 17, fontWeight: 700 }}>🔍 Inspeksi Kendaraan</div>
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 100px" }}>
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "22px 16px" }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#1a2a3a", marginBottom: 20 }}>🔍 Tujuan Inspeksi</div>
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#4a5568", display: "block", marginBottom: 12 }}>Tujuan Inspeksi</label>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {TUJUAN_INSPEKSI.map(t => (
                    <button key={t} onClick={() => setTujuan(t)}
                      style={{ width: "100%", padding: "16px 18px", borderRadius: 14, border: tujuan === t ? `2px solid ${ACCENT}` : "1.5px solid #e0e8f0", background: tujuan === t ? `rgba(42,92,42,0.07)` : "#fff", color: tujuan === t ? ACCENT : "#1a2a3a", fontWeight: tujuan === t ? 700 : 500, fontSize: 15, cursor: "pointer", textAlign: "left" as const, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span>{t}</span>
                      {tujuan === t && <span style={{ fontSize: 18 }}>✅</span>}
                    </button>
                  ))}
                </div>
              </div>
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  {(["mobil","motor"] as const).map(j => (
                    <button key={j} onClick={() => setJenisKendaraan(j)}
                      style={{ flex: 1, padding: "13px 8px", borderRadius: 14, border: jenisKendaraan === j ? `2px solid ${ACCENT}` : "2px solid #e0e8f0", background: jenisKendaraan === j ? `rgba(42,92,42,0.08)` : "#f8fafc", color: jenisKendaraan === j ? ACCENT : "#7a8a9a", fontWeight: 700, fontSize: 15, cursor: "pointer" }}>
                      {j === "mobil" ? "🚗 Mobil" : "🏍️ Motor"}
                    </button>
                  ))}
                </div>
              </div>
              <div style={{ marginBottom: 16 }}>
                ...
                <input type="text" value={merekModel} onChange={e => setMerekModel(e.target.value)} placeholder="Contoh: Toyota Avanza, Honda Beat" style={{ width: "100%", padding: "14px 16px", borderRadius: 14, border: "1.5px solid #e0e8f0", fontSize: 15, color: "#1a2a3a", background: "#f8fafc", outline: "none" }} />
              </div>
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 100px" }}>
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "20px 14px" }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#1a2a3a", marginBottom: 16 }}>📍 Lokasi Kendaraan</div>
              <div style={{ position: "relative", borderRadius: 16, overflow: "hidden", marginBottom: 20, height: 260 }}>
                ...
                <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -100%)", pointerEvents: "none", zIndex: 1000, display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ background: "#fff", borderRadius: 8, padding: "4px 10px", marginBottom: 4, boxShadow: "0 2px 8px rgba(0,0,0,0.18)" }}><div style={{ fontSize: 11, fontWeight: 700, color: "#1a2a3a", whiteSpace: "nowrap" }}>{isGeocoding ? "Memuat..." : "Posisi Kendaraan"}</div></div>
              ...
              {autoAddress && <div style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "10px 14px", background: `rgba(42,92,42,0.06)`, borderRadius: 12, marginBottom: 16, border: `1px solid rgba(42,92,42,0.15)` }}><span style={{ fontSize: 14, flexShrink: 0, marginTop: 1 }}>📍</span><span style={{ fontSize: 13, color: "#1a2a3a", lineHeight: 1.4 }}>{autoAddress}</span></div>}
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#4a5568", display: "block", marginBottom: 8 }}>Detail Lokasi / Panduan Akses</label>
                <textarea value={detailAlamat} onChange={e => setDetailAlamat(e.target.value)} placeholder="Depan showroom, area parkir B, dll..." rows={3} style={{ width: "100%", padding: "14px 16px", borderRadius: 14, border: "1.5px solid #e0e8f0", fontSize: 15, color: "#1a2a3a", background: "#f8fafc", outline: "none", resize: "none", lineHeight: 1.5 }} />
              </div>
            </div>
          </div>
      ...
      {step === 3 && (
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 100px" }}>
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "22px 16px" }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#1a2a3a", marginBottom: 24 }}>🔬 Cari Inspektor</div>
              ...
              {orderStatus === "pending" && (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20, padding: "24px 0 16px" }}>
                  ...
                  <div style={{ textAlign: "center" }}><div style={{ fontSize: 17, fontWeight: 700, color: "#1a2a3a", marginBottom: 6 }}>Mencari Inspektor Terdekat...</div><div style={{ fontSize: 13, color: "#7a8a9a" }}>Menghubungi inspektor bersertifikat di sekitar Anda.</div></div>
              ...
              {orderStatus === "accepted" && acceptedMitra && (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 18px", background: "#e8f8f2", borderRadius: 14, border: "1.5px solid #b2e8d4" }}><span style={{ fontSize: 22 }}>✅</span><div style={{ fontSize: 15, fontWeight: 700, color: "#1a7a6a" }}>Inspektor Ditemukan!</div></div>
                  <div style={{ border: "1.5px solid #e0e8f0", borderRadius: 18, padding: "18px 16px", background: "#fff" }}>
                    <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 14 }}>
                      <div style={{ width: 56, height: 56, borderRadius: 14, background: `rgba(42,92,42,0.08)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, flexShrink: 0 }}>🔍</div>
                  ...
                  {chatOpen && (
                    <div style={{ border: "1.5px solid #e0e8f0", borderRadius: 16, overflow: "hidden" }}>
                      ...
                      <div style={{ minHeight: 160, maxHeight: 220, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: 8, background: "#fff" }}>
                        {chatMessages.length === 0 ? (<div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "20px 0" }}><span style={{ fontSize: 32, opacity: 0.3 }}>💬</span><div style={{ fontSize: 12, color: "#b0bec5" }}>Mulai diskusi dengan inspektor</div></div>) : chatMessages.map(m => { const isMine = m.senderRole === "pengguna"; return (<div key={m.id} style={{ display: "flex", flexDirection: "column", alignItems: isMine ? "flex-end" : "flex-start" }}><div style={{ maxWidth: "78%", padding: "8px 12px", borderRadius: isMine ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: isMine ? `linear-gradient(135deg,${ACCENT2},${ACCENT})` : "#f0f4f8", color: isMine ? "#fff" : "#1a2a3a", fontSize: 13 }}>{m.message}</div></div>); })}
                  ...
                  <button onClick={() => setMitraConfirmed(true)} disabled={mitraConfirmed} style={{ width: "100%", padding: "16px", borderRadius: 14, border: "none", background: mitraConfirmed ? "#a5d6a7" : "linear-gradient(135deg, #2e7d32, #43a047)", color: "#fff", fontWeight: 700, fontSize: 15, cursor: mitraConfirmed ? "default" : "pointer" }}>{mitraConfirmed ? "✅ Inspektor Dikonfirmasi" : "✅ Setuju & Panggil Inspektor"}</button>
                  {!mitraConfirmed && <button onClick={async () => { if (orderId) await fetch(`/api/pengguna/orders/${orderId}`, { method: "DELETE", credentials: "include" }).catch(() => {}); if (orderPollRef.current) clearInterval(orderPollRef.current); if (chatPollRef.current) clearInterval(chatPollRef.current); setOrderId(null); setOrderNo(""); setOrderStatus("creating"); setAcceptedMitra(null); setChatMessages([]); setChatInput(""); setChatOpen(false); setMitraConfirmed(false); setStep(2); setTimeout(() => setStep(3), 50); }} style={{ width: "100%", padding: "14px", borderRadius: 14, border: "1.5px solid #e0e8f0", background: "#fff", color: "#4a5568", fontWeight: 600, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>🔄 Cari Inspektor Lain</button>}
                </div>
              )}
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, padding: "12px 14px 20px", background: "linear-gradient(to top, #f0f4f8 90%, transparent)", zIndex: 100 }}>
            {orderStatus === "accepted" ? (<div style={{ display: "flex", gap: 12 }}><button onClick={async () => { if (orderId) await fetch(`/api/pengguna/orders/${orderId}`, { method: "DELETE", credentials: "include" }).catch(() => {}); if (orderPollRef.current) clearInterval(orderPollRef.current); if (chatPollRef.current) clearInterval(chatPollRef.current); navigate("/dashboard/pengguna"); }} style={{ flex: 1, padding: "15px", borderRadius: 16, border: "1.5px solid #e8a0a0", background: "#fff5f5", color: "#c0392b", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>✕ Batalkan</button><button onClick={() => { if (mitraConfirmed) { setChatOpen(false); setStep(4); } }} disabled={!mitraConfirmed} style={{ flex: 2, padding: "15px", borderRadius: 16, border: "none", background: mitraConfirmed ? `linear-gradient(135deg, ${ACCENT2} 0%, ${ACCENT} 100%)` : "#d0d8e0", color: mitraConfirmed ? "#fff" : "#a0aab4", fontWeight: 700, fontSize: 15, cursor: mitraConfirmed ? "pointer" : "not-allowed" }}>Lanjut →</button></div>)
              : (<button disabled style={{ width: "100%", padding: "17px", borderRadius: 16, border: "none", background: "#c0d0dc", color: "#fff", fontWeight: 700, fontSize: 16, cursor: "not-allowed" }}>{orderStatus === "creating" ? "Membuat pesanan..." : "Menunggu Inspektor Menerima..."}</button>)}
          </div>
        </>
      )}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "0 0 100px" }}>
            <div style={{ position: "relative", width: "100%", height: 220 }}>
              ...
              <div style={{ position: "absolute", top: 12, right: 12, zIndex: 500, display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <div style={{ background: `rgba(42,92,42,0.9)`, backdropFilter: "blur(6px)", borderRadius: 12, padding: "8px 14px", color: "#fff", fontSize: 12, fontWeight: 600 }}>📏 {trackDist != null ? (trackDist < 1 ? `${Math.round(trackDist*1000)} m` : `${trackDist.toFixed(1)} km`) : `${acceptedMitra.dist < 1 ? `${Math.round(acceptedMitra.dist*1000)} m` : `${acceptedMitra.dist.toFixed(1)} km`}`}</div>
              </div>
              <div style={{ position: "absolute", top: 12, left: 12, zIndex: 500, background: ACCENT, borderRadius: 12, padding: "8px 14px", color: "#fff", fontSize: 12, fontWeight: 700 }}>🔍 Inspektor dalam perjalanan...</div>
            </div>
            <div style={{ padding: "16px 14px 0" }}>
              <div style={{ border: "1.5px solid #e0e8f0", borderRadius: 16, padding: "14px 16px", display: "flex", gap: 12, alignItems: "center", marginBottom: 16, background: "#fff" }}>
                ...
                <div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 700, color: "#1a2a3a" }}>{acceptedMitra.name}</div><div style={{ fontSize: 12, color: "#7a8a9a", marginTop: 2 }}>✅ Inspektor Bersertifikat RIDE</div><div style={{ fontSize: 12, color: "#9aa5b4", marginTop: 1 }}>{acceptedMitra.rating != null ? `⭐ ${acceptedMitra.rating}` : "⭐ Baru"} · {acceptedMitra.totalOrders} order</div></div>
              ...
              {chatOpen && (
                <div style={{ border: "1.5px solid #e0e8f0", borderRadius: 16, overflow: "hidden", marginBottom: 16 }}>
                  ...
                  <div style={{ minHeight: 120, maxHeight: 200, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: 8, background: "#fff" }}>
                    {chatMessages.map(m => { const isMine = m.senderRole === "pengguna"; return (<div key={m.id} style={{ display: "flex", flexDirection: "column", alignItems: isMine ? "flex-end" : "flex-start" }}><div style={{ maxWidth: "78%", padding: "8px 12px", borderRadius: isMine ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: isMine ? `linear-gradient(135deg,${ACCENT2},${ACCENT})` : "#f0f4f8", color: isMine ? "#fff" : "#1a2a3a", fontSize: 13 }}>{m.message}</div></div>); })}
                    <div ref={chatBottomRef} />
                  </div>
                  <div style={{ display: "flex", gap: 8, padding: "10px 12px", background: "#f8fafc", borderTop: "1px solid #f0f4f8" }}>
                    <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && sendChatMessage()} placeholder="Ketik pesan..." style={{ flex: 1, padding: "10px 14px", borderRadius: 12, border: "1.5px solid #e0e8f0", fontSize: 13, outline: "none", background: "#fff" }} />
              ...
              <div style={{ background: "#fff", border: "1.5px solid #e0e8f0", borderRadius: 16, padding: "16px", marginBottom: 16 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#1a2a3a", marginBottom: 14 }}>📍 Status Inspeksi</div>
                {(["menuju","tiba","pengerjaan","selesai"]).map((key, i) => {
                  const labels = ["Inspektor menuju lokasi Anda","Inspektor sudah tiba","Sedang proses inspeksi","Inspeksi selesai ✅"];
      ...
      {step === 5 && (() => {
        ...
        const pmLabel: Record<string,string> = { cash: "Bayar Tunai ke Inspektor", transfer: "Transfer Bank", qris: "Bayar via QRIS" };
        const pmDesc: Record<string,string> = { cash: `Siapkan uang tunai sebesar ${fmtIdr(finalTotal)}.`, transfer: `Transfer ke rekening inspektor sebesar ${fmtIdr(finalTotal)}.`, qris: `Scan QRIS inspektor sebesar ${fmtIdr(finalTotal)}.` };
          ...
          <>
            <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 120px" }}>
              <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "20px 16px" }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: "#1a2a3a", marginBottom: 14 }}>📋 Laporan & Pembayaran</div>
                <div style={{ background: "#f0faf7", border: "1.5px solid #b6e6d7", borderRadius: 14, padding: "12px 16px", marginBottom: 16, textAlign: "center" as const }}><div style={{ fontSize: 14, fontWeight: 800, color: "#1a7a6a" }}>✅ Inspeksi Selesai!</div><div style={{ fontSize: 12, color: "#4a9a7a", marginTop: 3 }}>Laporan akan dikirim via chat. Selesaikan pembayaran.</div></div>
                {!paymentData && <div style={{ background: "#f8fafc", borderRadius: 16, border: "1.5px solid #e0e8f0", padding: "24px 16px", textAlign: "center" as const, marginBottom: 16 }}><div style={{ fontSize: 28, marginBottom: 10 }}>⏳</div><div style={{ fontSize: 14, fontWeight: 700, color: "#1a2a3a", marginBottom: 6 }}>Menunggu inspektor mengisi rincian biaya...</div></div>}
                  ...
                  <>
                    ...
                    <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e0e8f0", padding: "14px 16px", marginBottom: 14 }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        ...
                        <button onClick={() => { const disc = VOUCHERS[voucherCode.toUpperCase()]; setVoucherDiscount(disc ?? 0); }} style={{ padding: "10px 18px", borderRadius: 10, border: "none", background: `linear-gradient(135deg, ${ACCENT2}, ${ACCENT})`, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Pakai</button>
                      </div>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>{(["cash","transfer","qris"] as const).map(m => (<button key={m} onClick={() => setPaymentMethodUser(m)} style={{ flex: 1, padding: "10px 0", borderRadius: 10, border: selectedMethod === m ? `2px solid ${ACCENT}` : "1.5px solid #e0e8f0", background: selectedMethod === m ? `rgba(42,92,42,0.07)` : "#fff", color: selectedMethod === m ? ACCENT : "#7a8a9a", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>{m === "cash" ? "Cash" : m === "transfer" ? "Transfer" : "QRIS"}</button>))}</div>
