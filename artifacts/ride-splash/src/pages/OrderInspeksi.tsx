                  ...
                  {chatOpen && (
                    <div style={{ border: "1.5px solid #e0e8f0", borderRadius: 16, overflow: "hidden" }}>
                      ...
                      <div style={{ minHeight: 160, maxHeight: 220, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: 8, background: "#fff" }}>
                        {chatMessages.length === 0 ? (<div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, padding: "20px 0" }}><span style={{ fontSize: 32, opacity: 0.3 }}>💬</span><div style={{ fontSize: 12, color: "#b0bec5" }}>Mulai diskusi dengan inspektor</div></div>) : chatMessages.map(m => { const isMine = m.senderRole === "pengguna"; return (<div key={m.id} style={{ display: "flex", flexDirection: "column", gap: 3 }}><span style={{ fontSize: 10, fontWeight: 700, color: isMine ? "#1a7a6a" : "#7a8a9a", textTransform: "uppercase" as const, letterSpacing: "0.5px" }}>{isMine ? "Saya" : "Mitra"}</span><div style={{ padding: "8px 12px", borderRadius: "4px 12px 12px 12px", background: isMine ? "#e8f5f2" : "#f0f4f8", borderLeft: `3px solid ${isMine ? "#1a7a6a" : "#c0cdd8"}`, color: "#1a2a3a", fontSize: 13, lineHeight: 1.4, whiteSpace: "pre-wrap" }}>{m.message}</div></div>); })}
              ...
              {chatOpen && (
                <div style={{ border: "1.5px solid #e0e8f0", borderRadius: 16, overflow: "hidden", marginBottom: 16 }}>
                  ...
                  <div style={{ minHeight: 120, maxHeight: 200, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: 8, background: "#fff" }}>
                    {chatMessages.map(m => { const isMine = m.senderRole === "pengguna"; return (<div key={m.id} style={{ display: "flex", flexDirection: "column", gap: 3 }}><span style={{ fontSize: 10, fontWeight: 700, color: isMine ? "#1a7a6a" : "#7a8a9a", textTransform: "uppercase" as const, letterSpacing: "0.5px" }}>{isMine ? "Saya" : "Mitra"}</span><div style={{ padding: "8px 12px", borderRadius: "4px 12px 12px 12px", background: isMine ? "#e8f5f2" : "#f0f4f8", borderLeft: `3px solid ${isMine ? "#1a7a6a" : "#c0cdd8"}`, color: "#1a2a3a", fontSize: 13, lineHeight: 1.4, whiteSpace: "pre-wrap" }}>{m.message}</div></div>); })}
                    <div ref={chatBottomRef} />
                  </div>
                  <div style={{ display: "flex", gap: 8, padding: "10px 12px", background: "#f8fafc", borderTop: "1px solid #f0f4f8" }}>
                    <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && sendChatMessage()} placeholder="Ketik pesan..." style={{ flex: 1, padding: "10px 14px", borderRadius: 12, border: "1.5px solid #e0e8f0", fontSize: 13, outline: "none", background: "#fff" }} />
