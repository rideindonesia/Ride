...
export default function OrderTowing() {
  ...
  const [detailTujuan, setDetailTujuan] = useState("");
  ...
  const canNext2 = (pinLat !== null || userLat !== null) && destLat !== null;
  ...
  // Peta tujuan derek — inisialisasi saat masuk step 2
  useEffect(() => {
    if (step !== 2 || !destMapRef.current || destLeafletMapRef.current) return;
  ...
  useEffect(() => {
    ...
    const tujuanFinal = destAutoAddress || "";
    const descFull = [
      bisaDinyalakan ? "Kendaraan masih bisa dinyalakan" : "Kendaraan tidak bisa dinyalakan",
      tujuanFinal ? `Tujuan derek: ${tujuanFinal}` : null,
      detailTujuan.trim() ? `Detail tujuan: ${detailTujuan.trim()}` : null,
    ].filter(Boolean).join(". ");
    (() => { const fd = new FormData(); fd.append("vehicleType", jenisKendaraan); fd.append("vehicleModel", merekModel); fd.append("vehicleYear", ""); fd.append("damageCategories", JSON.stringify(kondisi)); fd.append("description", descFull); fd.append("pickupAddress", autoAddress || "Lokasi yang dipilih"); fd.append("detailAlamat", detailAlamat); fd.append("pickupLat", String(pinLat ?? userLat ?? 0)); fd.append("pickupLng", String(pinLng ?? userLng ?? 0)); if (destLat !== null) fd.append("destLat", String(destLat)); if (destLng !== null) fd.append("destLng", String(destLng)); if (destAutoAddress) fd.append("destAddress", destAutoAddress + (detailTujuan.trim() ? ` (${detailTujuan.trim()})` : "")); fd.append("serviceType", "towing"); if (foto) fd.append("foto", foto); return fetch("/api/pengguna/orders", { method: "POST", credentials: "include", body: fd }); })().then(r => r.json()).then(d => {
  ...
  const sendChatMessage = useCallback(async () => {
    ...
    setChatSending(true);
    try {
      const r = await fetch(`/api/pengguna/chat/${orderId}`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ message: chatInput.trim(), role: "pengguna" }) });
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 100px" }}>
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "20px 14px" }}>
              ...
              {/* Map tujuan derek — selalu tampil */}
              <div style={{ position: "relative", borderRadius: 16, overflow: "hidden", marginBottom: 10, height: 210 }}>
                ...
                <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -100%)", pointerEvents: "none", zIndex: 1000, display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ background: "#fff", borderRadius: 8, padding: "4px 10px", marginBottom: 4, boxShadow: "0 2px 8px rgba(0,0,0,0.18)" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#1a2a3a", whiteSpace: "nowrap" }}>
                      {isDestGeocoding ? "Memuat..." : "Tujuan Derek"}
                    </div>
              ...
              {/* Alamat tujuan hasil geocode */}
              <div style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "10px 14px", background: `rgba(201,123,0,0.07)`, borderRadius: 12, marginBottom: 14, border: `1px solid rgba(201,123,0,0.25)` }}>
                <span style={{ fontSize: 14, flexShrink: 0, marginTop: 1 }}>🏁</span>
                <span style={{ fontSize: 13, color: "#1a2a3a", lineHeight: 1.4 }}>{isDestGeocoding ? "Memuat alamat..." : (destAutoAddress || "Menentukan lokasi...")}</span>
              </div>

              {/* Detail tujuan (opsional) */}
              <div style={{ fontSize: 13, fontWeight: 600, color: "#1a2a3a", marginBottom: 6 }}>Detail Tujuan</div>
              <textarea
                value={detailTujuan}
                onChange={e => setDetailTujuan(e.target.value)}
                placeholder="Bengkel Pak Budi, depan SPBU, dll. (opsional)"
                rows={2}
                style={{ width: "100%", padding: "12px 16px", borderRadius: 14, border: detailTujuan.trim() ? `1.5px solid ${ACCENT}` : "1.5px solid #e0e8f0", fontSize: 14, color: "#1a2a3a", background: "#f8fafc", outline: "none", resize: "none", lineHeight: 1.6, marginBottom: 14 }}
              />
