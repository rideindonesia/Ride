...
export default function OrderTowing() {
  ...
  useEffect(() => {
    ...
    fetch("/api/pengguna/orders", {
      ...
      body: JSON.stringify({
        vehicleType: jenisKendaraan, vehicleModel: merekModel, vehicleYear: "",
        damageCategories: kondisi, description: descFull,
            ...
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "22px 16px" }}>
              ...
              {orderStatus === "accepted" && acceptedMitra && (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                          ...
                          chatMessages.map(m => {
                            ...
                            return (
                              <div key={m.id} style={{ display: "flex", flexDirection: "column", alignItems: isMine ? "flex-end" : "flex-start" }}>
                                <div style={{ maxWidth: "78%", padding: "9px 13px", borderRadius: isMine ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: isMine ? `linear-gradient(135deg, ${ACCENT_DARK}, ${ACCENT})` : "#f0f4f8", color: isMine ? "#fff" : "#1a2a3a", fontSize: 13, lineHeight: 1.4 }}>
                                  {m.message}
                                </div>
                                <div style={{ fontSize: 10, color: "#b0bec5", marginTop: 2 }}>{new Date(m.createdAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}</div>
                              </div>
                            );
                          })
                        )}
                  ...
                  <button
                    onClick={() => setMitraConfirmed(true)}
