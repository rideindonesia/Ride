...
export type TripSvc = "goride" | "gocar" | "gosend" | "goshop";
...
export default function OrderTrip({ svc }: { svc: TripSvc }) {
  ...
  const [pickingMode, setPickingMode] = useState<"pickup" | "dest">("pickup");
  ...
  useEffect(() => {
    ...
    const lat = pickupLat ?? userLat ?? -1.2654;
    const lng = pickupLng ?? userLng ?? 116.8312;
    ...
    // Default: set pickup to current center if not already picked
    if (pickupLat == null) {
      setPickupLat(lat); setPickupLng(lng);
      setIsGeocoding(true);
      reverseGeocode(lat, lng).then(addr => { setPickupAddress(addr); setIsGeocoding(false); });
    }
    ...
    map.on("moveend", () => {
      ...
      setIsGeocoding(true);
      setPickingMode(mode => {
        if (mode === "pickup") { setPickupLat(center.lat); setPickupLng(center.lng); }
        else { setDestLat(center.lat); setDestLng(center.lng); }
        reverseGeocode(center.lat, center.lng).then(addr => {
          if (mode === "pickup") setPickupAddress(addr); else setDestAddress(addr);
  ...
  // Render pickup/dest markers (inactive point only, to avoid overlap with center pin)
  useEffect(() => {
    const map = leafletMapRef.current;
    if (step !== 2 || !map) return;
    const pickupIcon = L.divIcon({ html: '<div style="width:30px;height:30px;background:#1a7a6a;border:3px solid #fff;border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;font-size:14px;">📍</div>', iconSize: [30, 30], iconAnchor: [15, 30], className: "" });
    const destIcon = L.divIcon({ html: '<div style="width:30px;height:30px;background:#e53e3e;border:3px solid #fff;border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;font-size:14px;">🏁</div>', iconSize: [30, 30], iconAnchor: [15, 30], className: "" });

    // Pickup marker
    if (pickingMode !== "pickup" && pickupLat != null && pickupLng != null) {
      if (pickupMarkerRef.current) pickupMarkerRef.current.setLatLng([pickupLat, pickupLng]);
      else pickupMarkerRef.current = L.marker([pickupLat, pickupLng], { icon: pickupIcon }).addTo(map);
    } else if (pickupMarkerRef.current) {
      pickupMarkerRef.current.remove(); pickupMarkerRef.current = null;
    }

    // Destination marker
    if (pickingMode !== "dest" && destLat != null && destLng != null) {
      if (destMarkerRef.current) destMarkerRef.current.setLatLng([destLat, destLng]);
      else destMarkerRef.current = L.marker([destLat, destLng], { icon: destIcon }).addTo(map);
  ...
  useEffect(() => {
    ...
    const address = pickupAddress || "Lokasi yang dipilih";
    ...
    if (meta.needRecipient) {
      formData.append("recipientName", recipientName);
      formData.append("recipientPhone", recipientPhone);
    }
    if (meta.needItemNote) formData.append("itemNote", itemNote);

    fetch("/api/pengguna/orders", { method: "POST", credentials: "include", body: formData })
      .then(r => r.json())
      .then(d => {
        if (!d.orderId) {
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 100px" }}>
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "22px 16px" }}>
              ...
              {meta.needRecipient && (
                <>
                  <div style={{ marginBottom: 20 }}>
                    <label style={{ fontSize: 13, fontWeight: 600, color: "#4a5568", display: "block", marginBottom: 8 }}>Nama Penerima</label>
                    <input type="text" value={recipientName} onChange={e => setRecipientName(e.target.value)} placeholder="Contoh: Budi Santoso" style={{ width: "100%", padding: "14px 16px", borderRadius: 14, border: "1.5px solid #e0e8f0", fontSize: 15, color: "#1a2a3a", background: "#f8fafc", outline: "none" }} />
                  </div>
                  <div style={{ marginBottom: 20 }}>
                    <label style={{ fontSize: 13, fontWeight: 600, color: "#4a5568", display: "block", marginBottom: 8 }}>No. HP Penerima</label>
                    <input type="tel" value={recipientPhone} onChange={e => setRecipientPhone(e.target.value)} placeholder="Contoh: 08123456789" style={{ width: "100%", padding: "14px 16px", borderRadius: 14, border: "1.5px solid #e0e8f0", fontSize: 15, color: "#1a2a3a", background: "#f8fafc", outline: "none" }} />
                  </div>
                </>
              )}
              ...
              {meta.needItemNote && (
                <div style={{ marginBottom: 20 }}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "#4a5568", display: "block", marginBottom: 8 }}>{meta.itemNoteLabel}</label>
                  <textarea value={itemNote} onChange={e => setItemNote(e.target.value)} placeholder={meta.itemNotePlaceholder} rows={5} style={{ width: "100%", padding: "14px 16px", borderRadius: 14, border: "1.5px solid #e0e8f0", fontSize: 15, color: "#1a2a3a", background: "#f8fafc", outline: "none", resize: "none", lineHeight: 1.5 }} />
                </div>
              )}
        ...
        <>
          <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 100px" }}>
            <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "20px 14px" }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#1a2a3a", marginBottom: 16 }}>🗺️ Pilih Rute</div>
                ...
                <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -100%)", pointerEvents: "none", zIndex: 1000, display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ background: "#fff", borderRadius: 8, padding: "4px 10px", marginBottom: 4, boxShadow: "0 2px 8px rgba(0,0,0,0.18)" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#1a2a3a", whiteSpace: "nowrap" }}>
                      {isGeocoding ? "Memuat..." : pickingMode === "pickup" ? "Titik Jemput" : "Titik Tujuan"}
                    </div>
                    {!isGeocoding && <div style={{ fontSize: 10, color: "#7a8a9a", whiteSpace: "nowrap" }}>Geser untuk sesuaikan</div>}
                  </div>
                  <span style={{ fontSize: 32, filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.3))" }}>{pickingMode === "pickup" ? "📍" : "🏁"}</span>
                </div>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "10px 14px", background: "rgba(26,122,106,0.07)", borderRadius: 12, marginBottom: 10, border: "1px solid rgba(26,122,106,0.2)" }}>
                <span style={{ fontSize: 14, flexShrink: 0, marginTop: 1 }}>📍</span>
                <div>
                  <div style={{ fontSize: 10, color: "#1a7a6a", fontWeight: 700 }}>TITIK JEMPUT</div>
                  <div style={{ fontSize: 13, color: "#1a3a5c", lineHeight: 1.4 }}>{pickupAddress || "Belum dipilih"}</div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "10px 14px", background: "rgba(229,62,62,0.06)", borderRadius: 12, marginBottom: 16, border: "1px solid rgba(229,62,62,0.2)" }}>
                <span style={{ fontSize: 14, flexShrink: 0, marginTop: 1 }}>🏁</span>
                <div>
                  <div style={{ fontSize: 10, color: "#e53e3e", fontWeight: 700 }}>TITIK TUJUAN</div>
                  <div style={{ fontSize: 13, color: "#1a3a5c", lineHeight: 1.4 }}>{destAddress || "Belum dipilih — tap tombol Titik Tujuan"}</div>
                </div>
              </div>
