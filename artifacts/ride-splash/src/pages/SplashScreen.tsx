...
export default function SplashScreen() {
  ...
  useEffect(() => {
    if (isSuccess && data) {
      let role = (data as { role?: string })?.role;
      try {
        const lastRole = localStorage.getItem("ride-last-role");
        if (lastRole === "pengguna" || lastRole === "mitra") role = lastRole;
      } catch { /* ignore */ }
      const timer = setTimeout(() => {
        if (role === "pengguna") navigate("/dashboard/pengguna");
